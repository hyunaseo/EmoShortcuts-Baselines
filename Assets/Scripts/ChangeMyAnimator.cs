...
public class ChangeMyAnimator : MonoBehaviour
{
    ...
    private RetargetingLayer retargetingLayer;
    ...
    void Update(){
        if (runtimeAvatarLoadChecker.loadedAvatar != null && myAvatar == null)
        {
            myAvatar = runtimeAvatarLoadChecker.loadedAvatar;
            animator = myAvatar.GetComponent<Animator>();
            retargetingLayer = myAvatar.GetComponent<RetargetingLayer>();
            ovrBody = myAvatar.GetComponent<OVRBody>();
        }
    }
